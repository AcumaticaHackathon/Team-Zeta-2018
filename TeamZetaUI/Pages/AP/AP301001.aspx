<%@ Page Language="C#" MasterPageFile="~/MasterPages/FormDetail.master" AutoEventWireup="true" ValidateRequest="false" CodeFile="AP301001.aspx.cs" Inherits="Page_AP301001" Title="Untitled Page" %>

<%@ MasterType VirtualPath="~/MasterPages/FormDetail.master" %>

<asp:Content ID="cont1" ContentPlaceHolderID="phDS" runat="Server">
    <px:PXDataSource ID="ds" runat="server" Visible="True" Width="100%" TypeName="TeamZeta.APInvoiceEntryPXInhExt" PrimaryView="Document">
...
<asp:Content ID="cont2" ContentPlaceHolderID="phF" runat="Server">
    <script type="text/javascript">
        function commandResult(ds, context) {
            ...
            if (context.command != "Insert") {
                for (var key in ds.px_context.px_all) {
                    if (key.indexOf("_ExternalFilePanel") > 0) {
                        var tabControl = ds.px_context.px_all[key];
                    }
                    if (key.indexOf("_PXUsrFileURL") > 0) {
                        var urlControl = ds.px_context.px_all[key];
                    }
                    if (key.indexOf("_sp0") > 0) {
                        var ft = px_context.px_all['ctl00_phF_sp0'];
                    }
                }
                var url = urlControl.getValue();
        ...
        function hidePanel() {
            var tabControl;
            var __showNavigationPane = 'Show PDF Pane';
            var __hideNavigationPane = 'Hide PDF Pane';

            for (var key in px_context.px_all) {

                if (key.indexOf("_ExternalFilePanel") > 0) {
                    var tabControl = px_context.px_all[key];
                }
                if (key.indexOf("_PXUsrFileURL") > 0) {
                    var urlControl = px_context.px_all[key];
                }
                if (key.indexOf("_sp0") > 0) {
                    var ft = px_context.px_all['ctl00_phF_sp0'];
                }
                if (key.indexOf("_PXUsrContainerSize") > 0) {
                    var containerSize = px_context.px_all[key];
                }
            }

            var hideImg = px.elemByID("hideFrame");
            var url = urlControl.getValue();
            var containerSizeValue = containerSize.getValue();
            if (url.includes('00000000-0000-0000-0000-000000000000')) {
                url = '';
            }
           
            if (hideImg && hideImg.getAttribute("icon") == "ShowFrame") {
                if (tabControl) {
                    tabControl.setPageUrl('');
                    tabControl.repaint();
                }
                if (ft) {
                    containerSizeValue = ft.getPanel2Size();
                    containerSize.setValue(containerSizeValue.toString());
                    ft.setPanel2Size('2');
                }
                if (hideImg) {
                    px.setTitle(hideImg.parentNode, __showNavigationPane);
                    px_cm.repaintImage(hideImg, "control@HideFrame");
                }
            }
            else {
                if (tabControl) {
                ...
                if (ft) {
                    if (containerSizeValue !=  '')
                        ft.setPanel2Size(containerSizeValue);
                    else
                        ft.setPanel2Size('550');
                }
                if (hideImg) {
                    px.setTitle(hideImg.parentNode, __hideNavigationPane);
                    px_cm.repaintImage(hideImg, "control@ShowFrame");
                }
            }
        }
    </script>
    <style type="text/css">
        .hidePDFFrameBox {
            ...
            .hidePDFFrameBox:hover {
                background-color: #bdbdbd;
                cursor: pointer;
            }
    </style>
    <px:PXSplitContainer runat="server" ID="sp0" SplitterPosition="932">
        ...
        <Template1>
            <px:PXFormView ID="form" runat="server" Style="z-index: 100" Width="100%"
                ...
                LinkIndicator="True" NotifyIndicator="True" DefaultControlID="edDocType" AllowCollapse="true"
                ...
                <Template>
                    ...
                    <px:PXTextEdit ID="PXUsrFileURL" runat="server" DataField="UsrFileURL" AlreadyLocalized="False" DefaultLocale="" />
                    <px:PXTextEdit ID="PXUsrContainerSize" runat="server" DataField="UsrContainerSize"  AlreadyLocalized="False" DefaultLocale=""  />
                    <px:PXLayoutRule runat="server" StartColumn="True" LabelsWidth="S" ControlSize="S" />
            ...
            <px:PXSmartPanel ID="PanelAddPOReceiptLine" runat="server" Caption="Add Receipt Line" CaptionVisible="True"
                Height="400px" Width="900px" Key="poReceiptLinesSelection" LoadOnDemand="True" ShowAfterLoad="True" AutoCallBack-Command="Refresh" AutoCallBack-Target="gridOL" AutoReload="True">
        ...
        <Template2>
            <div runat="server" id="hidePDFFrameBox" class="hidePDFFrameBox" onclick="hidePanel()" title="Hide PDF Pane">
                <px:PXImage runat="server" ID="hideFrame" ImageUrl="control@ShowFrame"></px:PXImage>
            </div>
            <px:PXSmartPanel ID="ExternalFilePanel" runat="server" SkinID="Frame"
                AllowMove="false" AllowResize="true" AutoSize-Enabled="true"  AutoReload="true" AutoRepaint="true" 
                LoadOnDemand="true" RenderIFrame="true" RenderVisible="True">
            </px:PXSmartPanel>
        </Template2>
    </px:PXSplitContainer>
</asp:Content>